...
def check_state(state: str) -> bool:
    ...
    return state in list(available_locations.keys())


def get_graphql_query(licence_number: str, state: str) -> dict:

    if (not licence_number and not state) or (not check_state(state)):
        raise RuntimeError("licence_number and state are required!!!")

    query = {
        "operationName": "licensePlate",
        "variables": {"number": f"{licence_number}", "state": f"{state}"},
        "query": "query licensePlate($number: String!, $state: String!) {\n  licensePlate(number: $number, state: $state) {\n    vin\n    __typename\n  }\n}\n",
    }
    return query
...
def get_vin_number(query_data: dict, useragent: str, proxies: dict, cloudflare=False):
    ...
    if cloudflare:
        logger.info("Enabled Cloudflare's anti-bot page bypass.")
    ...
    if useragent:
        ...
        session.headers["Origin"] = "https://www.vehiclehistory.com"

    if isinstance(query_data, dict):
        query_data = json.dumps(query_data)

    try:
        r = session.post(url=url, data=query_data)
        assert r.status_code == 200
...
def arg_parser():
    ...
    parser.add_argument(
        "--licence-number", required=True, type=str, help="Vehicle licence number."
    )
    parser.add_argument(
        "--state",
        required=True,
        type=str,
        help=(
            "A state where licence is registered.\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Node.js version 10 or above is required to interpret Cloudflare's "
            "obfuscated JavaScript challenge.\n"
            "Your machine may already have Node installed (check with node -v).\n"
            "If not, you can install it with apt-get install nodejs on Ubuntu >= 18.04 "
            "and Debian >= 9 and brew install node on macOS.\n"
        ),
    )
...
if __name__ == "__main__":
    ...
    query_data = get_graphql_query(
        args.get("licence_number"), args.get("state").upper()
    )
    vin = get_vin_number(
        query_data=query_data,
        useragent=useragent,
        proxies=proxies,
        cloudflare=args["spoof_cloudflare"],
    )
    print(vin)
